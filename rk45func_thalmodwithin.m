...
%% THIS FUNCTION IMPLEMENTS THE RK45 ALGORITHM FOR THE KINETIC MODEL  BEING WORKED ON BY DAN AND TOM. FEBRUARY 2014.
...
%     % Tval=[Tmax Kp Vp];
    ...
    T_tcr=Tval(1)/(1+exp(-(V_tcr-Tval(3))/Tval(2))); %% transmitter concentration corresponding to the TCR synapse
    T_trn=Tval(1)/(1+exp(-(V_trn-Tval(3))/Tval(2))); %% transmitter concentration corresponding to the TRN synapse
    T_in=Tval(1)/(1+exp(-(V_in-Tval(3))/Tval(2))); %% transmitter concentration corresponding to the IN synapse
%     % ampaval=[E_ampa alpha_ampa beta_ampa g_ampa_tcr2trn g_ampa_ret2tcr g_ampa_ret2in];
%     % gabaAval=[alpha_gaba_a beta_gaba_a g_gaba_a_trn2tcr g_gaba_a_trn2trn E_gaba_a_trn2tcr E_gaba_a_trn2trn g_gaba_a_in2tcr g_gaba_a_in2in E_gaba_a_in2tcr E_gaba_a_in2in];
%     % gabaBval=[alpha1_gaba_b beta1_gaba_b alpha2_gaba_b   beta2_gaba_b g_gaba_b E_gaba_b Kd_gaba_b n];
%     % leakval=[E_leak_tcr g_leak_tcr E_leak_trn g_leak_trn E_leak_in
